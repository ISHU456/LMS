...
import {
  ...
  ShieldCheck, FileCheck, Settings, Search, Megaphone, ClipboardCheck, Terminal, ClipboardList, Wind, Leaf, Droplets, Zap, Building
} from 'lucide-react';
      ...
      <div className="bg-white/90 dark:bg-[#080c14]/90 backdrop-blur-2xl rounded-[3rem] shadow-2xl w-full max-w-2xl border border-slate-200 dark:border-white/5 overflow-hidden flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
        ...
        <div className="px-10 py-8 flex items-center justify-between border-b border-slate-100 dark:border-white/5 shrink-0">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h2 className="text-lg font-bold text-slate-900 dark:text-white uppercase tracking-tighter">{isEdit ? 'Re-calibrate Protocol' : 'Induct New Session'}</h2>
              <p className="text-[10px] text-teal-500 font-bold uppercase tracking-widest mt-0.5 flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-3">
              ...
              <select 
                ...
                className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl px-5 py-4 text-xs font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-teal-500/50 outline-none transition-all appearance-none cursor-pointer"
              >
              ...
              <div className="flex gap-2 p-1 bg-slate-50 dark:bg-white/5 rounded-2xl border border-slate-200 dark:border-white/10">
                {['Theory', 'Practical'].map(type => (
                  <button 
                    ...
                    className={`flex-1 py-3 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${form.type === type ? 'bg-white dark:bg-teal-500 text-teal-600 dark:text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}
                  >
                    {type}
                  </button>
                ))}
             ...
             <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
                {DAYS.map(d => (
                   <button 
                     ...
                     className={`py-3 rounded-xl text-[9px] font-bold uppercase tracking-widest transition-all border ${form.day === d ? 'bg-indigo-600 border-indigo-600 text-white shadow-xl shadow-indigo-600/20' : 'bg-transparent border-slate-200 dark:border-white/5 text-slate-400 hover:border-indigo-500/30'}`}
                   >
                     {d.slice(0, 3)}
                   </button>
                ))}
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start mb-1">
                         <span className={`text-[8px] font-bold uppercase tracking-widest ${isSelected ? 'text-teal-100' : 'text-slate-400'}`}>{slot.label}</span>
                         {conflict && <AlertCircle size={10} className="text-rose-500" />}
                      </div>
                      <p className="text-xs font-bold tracking-tight">{slot.start} — {slot.end}</p>
                      {conflict && (
                        <div className="absolute inset-0 bg-white/10 dark:bg-black/20 flex items-center justify-center opacity-0 group-hover/slot:opacity-100 transition-opacity rounded-2xl">
                           <p className="text-[7px] font-bold uppercase bg-rose-500 text-white px-2 py-1 rounded-lg shadow-lg">Room Conflict: {conflict.course}</p>
                        </div>
                      )}
                    </button>
                  );
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-3">
              ...
              <div className="relative">
                <input 
                  ...
                  className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl px-5 py-4 text-xs font-bold text-slate-900 dark:text-white placeholder:text-slate-300 dark:placeholder:text-slate-600 focus:ring-2 focus:ring-teal-500/50 outline-none transition-all"
                />
            ...
            <div className="space-y-3">
              ...
              <textarea 
                ...
                rows={1} 
                className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl px-5 py-4 text-xs font-bold text-slate-900 dark:text-white placeholder:text-slate-300 dark:placeholder:text-slate-600 focus:ring-2 focus:ring-teal-500/50 outline-none transition-all resize-none"
              />
            </div>
          </div>
        </div>
        ...
        <div className="px-10 py-8 border-t border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/5 shrink-0 flex gap-4">
          <button onClick={onClose} className="flex-1 py-4 rounded-2xl border border-slate-200 dark:border-white/10 text-slate-500 dark:text-slate-400 font-bold text-[10px] uppercase tracking-[0.2em] hover:bg-white dark:hover:bg-white/10 transition-all">Abort</button>
          <button 
            disabled={!form.course || !form.startTime || !form.room}
            onClick={onSave} 
            className="flex-1 py-4 rounded-2xl bg-gradient-to-r from-teal-600 to-indigo-600 text-white font-bold text-[10px] uppercase tracking-[0.2em] flex items-center justify-center gap-3 shadow-2xl shadow-indigo-500/30 transition-all hover:scale-[1.02] active:scale-95 disabled:opacity-30 disabled:scale-100 disabled:grayscale"
          >
    ...
    <motion.div layout initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, x:-20 }} className="bg-white/70 dark:bg-[#080c14]/70 backdrop-blur-xl rounded-[2.5rem] border border-slate-200 dark:border-white/5 shadow-xl hover:shadow-2xl hover:border-teal-500/30 transition-all group overflow-hidden">
      ...
      <div className="p-8 flex items-center gap-6">
        <div className="shrink-0 text-center w-24 bg-slate-50 dark:bg-white/5 rounded-2xl py-4 border border-slate-100 dark:border-white/5 shadow-inner">
          <p className="text-sm font-bold text-slate-900 dark:text-white tracking-tighter tabular-nums">{s.startTime}</p>
          <div className="w-6 h-[2px] bg-teal-500/30 mx-auto my-2 rounded-full"/>
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest tabular-nums">{s.endTime}</p>
        </div>
        
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 mb-2 flex-wrap">
            <h4 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-tight italic">{s.course}</h4>
            <span className="flex items-center gap-1 text-[8px] font-bold px-3 py-1 rounded-lg uppercase tracking-widest border border-current opacity-70" style={{ color: typeMeta.color }}>
              <TypeIcon size={10}/> {s.type}
            </span>
          </div>
          <div className="flex items-center gap-5 flex-wrap">
            <span className="flex items-center gap-2 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest italic animate-pulse">
               <div className="w-1.5 h-1.5 rounded-full bg-teal-500 shadow-[0_0_8px_rgba(20,184,166,0.6)]" /> 
               {s.room}
            </span>
            {s.notes && (
               <span className="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest truncate max-w-[200px]">
...
const FacultyDashboard = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const [assignmentsRes, announcementsRes, coursesRes] = await Promise.all([
          axios.get('http://localhost:5001/api/assignments', config),
        ...
        if (lastCourseId) {
          const last = coursesData.find(c => c._id === lastCourseId);
  ...
  if (isLoading) return <GeminiLoader fullScreen text="Synchronizing Academic Grid..." />;
          ...
          <div className="p-6 flex flex-col h-full">
             <div className="flex items-center gap-3 mb-10 px-2 overflow-hidden">
                ...
                <div className="truncate">
                   <h2 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-tighter">Faculty Node</h2>
                   <p className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest italic">Academic Grid</p>
                </div>
             ...
             <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-800/60 overflow-hidden">
                <div className="p-4 bg-slate-50 dark:bg-white/5 rounded-2xl border border-slate-100 dark:border-white/5 group hover:border-indigo-500/30 transition-all duration-500 cursor-help">
                   <div className="flex items-center gap-3 mb-2">
                      ...
                      <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">Node Health</p>
                   </div>
                   <div className="w-full bg-slate-200 dark:bg-slate-800 h-1.5 rounded-full overflow-hidden">
                      <div className="h-full bg-gradient-to-r from-indigo-500 to-indigo-500 w-[96%]" />
                   </div>
                   <p className="text-[8px] font-bold text-indigo-500 uppercase tracking-widest mt-2">Operational: 100%</p>
                </div>
             </div>
          </div>
        ...
        <main className="flex-1 p-4 lg:p-8 overflow-y-auto no-scrollbar min-w-0">
          <div className="flex lg:hidden items-center justify-between mb-6">
             <button 
               ...
               className="flex items-center gap-2 px-4 py-2 rounded-xl bg-indigo-600 text-white font-bold text-[10px] uppercase tracking-widest shadow-lg shadow-indigo-600/20"
             >
               <Activity size={14} />
               Menu
             </button>
             <div className="text-[10px] font-bold uppercase text-indigo-500">Pulse Node 001</div>
          </div>
            ...
            >
              ...
              <div className="relative z-10 drop-shadow-sm max-w-full overflow-visible">
                <div className="flex items-center gap-4 mb-8">
                   ...
                   <span className="text-[11px] font-bold text-teal-600 dark:text-teal-400 uppercase tracking-[0.6em] italic drop-shadow-sm flex items-center gap-2">
                     <Wind size={14}/> Faculty Governance Matrix
                   </span>
                </div>
                
                <h1 className="text-4xl lg:text-5xl xl:text-6xl font-bold text-slate-900 dark:text-white uppercase tracking-tighter leading-[0.85] mb-8 drop-shadow-2xl max-w-4xl">
                  Welcome, <br />
                ...
                <div className="max-w-3xl border-l-[3px] border-teal-500/40 pl-8 mb-10 transform group-hover:translate-x-1 transition-transform duration-700">
                  <p className="text-slate-600 dark:text-slate-400 text-[11px] lg:text-[13px] font-bold uppercase tracking-[0.3em] leading-relaxed italic opacity-85">
                ...
                <div className="flex flex-wrap items-center gap-4">
                  <div className="flex items-center gap-3 px-8 py-4 bg-white/80 dark:bg-white/5 backdrop-blur-2xl rounded-[1.75rem] border border-slate-200 dark:border-white/5 shadow-2xl shadow-indigo-500/10 group/time">
                    ...
                    <p className="text-slate-900 dark:text-white text-[11px] font-bold uppercase tracking-[0.2em]">{time.toLocaleDateString('en-IN', { day: '2-digit', month: '2-digit', year: 'numeric' })}</p>
                  </div>
                  <div className="flex items-center gap-3 px-8 py-4 bg-white/80 dark:bg-white/5 backdrop-blur-2xl rounded-[1.75rem] border border-slate-200 dark:border-white/5 shadow-2xl shadow-indigo-500/10 group/time">
                    <Clock size={18} className="text-indigo-500 group-hover/time:rotate-12 transition-transform" />
                    <p className="text-slate-900 dark:text-white text-[11px] font-bold uppercase tracking-[0.2em] tabular-nums">{time.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</p>
                  </div>
                  ...
                  <div className="flex items-center gap-3 px-8 py-4 bg-indigo-600/10 dark:bg-indigo-500/10 backdrop-blur-xl rounded-[1.75rem] border border-indigo-500/20 shadow-2xl shadow-indigo-500/5 group/credits">
                    ...
                    <p className="text-indigo-600 dark:text-indigo-400 text-[11px] font-bold uppercase tracking-[0.2em]">Institutional Focus: <span className="text-indigo-500 ml-1 font-bold">{settings.facultyNeuralCredit} Node Credits</span></p>
                  </div>
                </div>
              </div>
              ...
              <div className="absolute top-12 right-12 z-30 hidden lg:flex items-center gap-3 px-5 py-2.5 bg-white/40 dark:bg-white/5 backdrop-blur-2xl rounded-2xl border border-white/20 dark:border-white/10 shadow-xl">
                 ...
                 <span className="text-[9px] font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-[0.4em] italic">Neural Sync Active</span>
              </div>
            </motion.div>
          )}
             ...
             {activeTab === 'schedule' && (
                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-10">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-12">
                    <div>
                      <h2 className="text-3xl font-bold uppercase tracking-tighter dark:text-white italic">Academic Schedule <span className="text-teal-500">Matrix</span></h2>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.3em] mt-2">Synchronizing institutional knowledge nodes</p>
                    </div>
                    <button onClick={openAdd} className="px-8 py-4 bg-gradient-to-r from-teal-600 to-indigo-600 text-white rounded-[1.5rem] text-[10px] font-bold uppercase tracking-[0.2em] shadow-2xl shadow-teal-500/20 hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-3">Induct Session <Plus size={16}/></button>
                  </div>
                  ...
                  <div className="flex gap-3 mb-10 overflow-x-auto pb-4 scrollbar-none scroll-smooth">
                    {['All', ...DAYS].map(d => (
                      <button 
                        ...
                        className={`px-7 py-3 rounded-2xl text-[9px] font-bold uppercase tracking-widest transition-all border ${filterDay === d ? 'bg-indigo-600 border-indigo-600 text-white shadow-xl shadow-indigo-600/20' : 'bg-white dark:bg-[#080c14] text-slate-400 border-slate-100 dark:border-white/5 hover:border-indigo-500/30'}`}
                      >
                        {d}
                      </button>
                    ))}
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-8 pb-12">
                    ...
                    {filteredSchedule.length === 0 && (
                      <div className="col-span-full py-32 bg-white/40 dark:bg-white/5 border border-dashed border-slate-200 dark:border-white/10 rounded-[4rem] flex flex-col items-center justify-center opacity-60">
                         ...
                         <h3 className="text-sm font-bold uppercase tracking-[0.4em] text-slate-400 italic mb-2">Temporal Void Detected</h3>
                         <p className="text-[9px] font-bold uppercase tracking-widest text-slate-500">No active operational sessions in this sector</p>
                      </div>
                    )}
                  </div>
                </motion.div>
              ...
              {activeTab === 'quizzes' && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                  ...
                  <div className="flex items-center justify-between mb-6">
                    <div>
                      <h2 className="text-xl font-bold text-gray-900 dark:text-white uppercase italic">Active Neural Quiz Nodes</h2>
                      <p className="text-[10px] text-indigo-500 font-bold uppercase tracking-widest mt-1">Gamification & Achievement Interface</p>
                    </div>
                    <button onClick={() => setQuizGenOpen(true)} className="px-6 py-2.5 bg-indigo-600 text-white rounded-xl text-[10px] font-bold uppercase tracking-widest shadow-lg hover:bg-indigo-500 transition-all flex items-center gap-2">Deploy New Quiz <Plus size={14}/></button>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {quizzes.map(q => (
                      <div key={q._id} className="bg-white dark:bg-[#080c14] p-6 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-md transition-all">
                        <div className="flex items-center gap-4 mb-4">
                           ...
                           <div>
                              <h3 className="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-tight">{q.title}</h3>
                              <span className="text-[9px] font-bold text-gray-500 uppercase tracking-widest">{q.category}</span>
                           </div>
                        </div>
                        <div className="flex items-center gap-6 mt-6">
                           <div className="flex items-center gap-2">
                              {/* <Target size={14} className="text-gray-400" /> */}
                              <span className="text-[10px] font-bold text-gray-500 uppercase">{q.totalPoints} Pts</span>
                           </div>
                           <div className="flex items-center gap-2">
                              <Clock size={14} className="text-gray-400" />
                              <span className="text-[10px] font-bold text-gray-500 uppercase">{q.timeLimit} Min</span>
                           </div>
                        </div>
                      </div>
                    ))}
