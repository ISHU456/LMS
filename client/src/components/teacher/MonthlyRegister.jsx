...
const MonthlyRegister = ({ user, initialSemester, initialCourse, onPersistChange }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white/80 dark:bg-[#080c14] border border-gray-100 dark:border-gray-800 rounded-[2.5rem] shadow-xl relative">
        ...
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full border-separate border-spacing-0">
            <thead className="sticky top-0 z-[45] bg-slate-900 dark:bg-black border-b border-white/10">
              <tr>
                <th className="px-4 py-4 text-left w-12 sticky left-0 top-0 z-50 bg-slate-900 dark:bg-black border-b border-r border-white/10 text-[9px] lg:text-[10px] font-black uppercase tracking-widest text-white">#</th>
                <th className="px-4 lg:px-6 py-4 text-left min-w-[100px] lg:min-w-[120px] sticky left-[48px] top-0 z-50 bg-slate-900 dark:bg-black border-b border-r border-white/10 text-[9px] lg:text-[10px] font-black uppercase tracking-widest text-white">Roll No</th>
                <th className="px-4 lg:px-6 py-4 text-left min-w-[160px] lg:min-w-[200px] sticky left-[148px] lg:left-[168px] top-0 z-50 bg-slate-900 dark:bg-black border-b border-r border-white/10 text-[9px] lg:text-[10px] font-black uppercase tracking-widest text-white">Identity</th>
                {daysInMonth.map(day => (<th key={day} className="px-2 py-4 min-w-[35px] lg:min-w-[40px] border-b border-r border-white/10 text-[9px] lg:text-[10px] font-black uppercase text-white tabular-nums text-center bg-slate-900 dark:bg-black sticky top-0 z-[44]">{day}</th>))}
              </tr>
            </thead>
            <tbody>
              {paginatedStudents.map((student, idx) => {
                ...
                return (
                  <tr key={`reg-${student._id}`} className="hover:bg-gray-100 dark:hover:bg-gray-800 border-b border-gray-100 dark:border-gray-800">
                    <td className="px-4 py-3 sticky left-0 z-10 bg-white dark:bg-[#080c14] border-r border-gray-100 dark:border-gray-700 font-black text-[10px] lg:text-[11px] text-gray-400 tabular-nums">{(currentPage - 1) * itemsPerPage + idx + 1}</td>
                    <td className="px-4 lg:px-6 py-3 sticky left-[48px] z-10 bg-white dark:bg-[#080c14] border-r border-gray-100 dark:border-gray-700 font-black text-[10px] lg:text-[11px] text-gray-400">{student.rollNumber}</td>
                    <td className="px-4 lg:px-6 py-3 sticky left-[148px] lg:left-[168px] z-10 bg-white dark:bg-[#080c14] border-r border-gray-100 dark:border-gray-700 font-black text-[11px] lg:text-xs uppercase cursor-pointer italic truncate max-w-[160px] lg:max-w-none" onClick={() => setViewingStudentId(student._id)}>{student.name}</td>
                    {daysInMonth.map(day => {
        ...
        {Math.ceil(filteredStudents.length / itemsPerPage) > 1 && (
          <div className="px-8 py-6 border-t border-gray-50 dark:border-gray-800 flex items-center justify-between">
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest italic">
              Registry Block {currentPage} of {Math.ceil(filteredStudents.length / itemsPerPage)}
            </p>
