...
const AdminFacultyMonthlyRegister = ({ user }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Grid Section */}
      <div className="bg-white dark:bg-[#080c14] rounded-[3rem] shadow-xl border border-slate-200 dark:border-slate-800/60 overflow-hidden relative">
        {isLoading && (
        ...
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full border-collapse">
            <thead className="bg-slate-50/90 dark:bg-slate-800/90 backdrop-blur-md sticky top-0 z-10">
              <tr>
                <th className="px-6 py-5 text-left min-w-[120px] sticky left-0 z-20 bg-slate-50 dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 text-[10px] font-black uppercase tracking-widest text-slate-400">Emp ID</th>
                <th className="px-6 py-5 text-left min-w-[220px] sticky left-[120px] z-20 bg-slate-50 dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 text-[10px] font-black uppercase tracking-widest text-slate-400">Faculty Identity</th>
                {daysInMonth.map(day => (
                  <th key={day} className="px-2 py-5 min-w-[45px] border-r border-slate-200 dark:border-slate-700 text-[10px] font-black uppercase text-slate-500 tabular-nums text-center">{day}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {paginatedFaculty.length > 0 ? paginatedFaculty.map((member, idx) => {
                ...
                return (
                  <motion.tr 
                    ...
                    className="hover:bg-slate-50 dark:hover:bg-indigo-500/5 transition-all border-b border-slate-100 dark:border-slate-800/60"
                  >
                    <td className="px-6 py-4 sticky left-0 z-10 bg-white/95 dark:bg-[#080c14]/95 backdrop-blur-md border-r border-slate-200 dark:border-slate-700 font-black text-[10px] text-slate-400 font-mono tracking-tighter italic uppercase">{member.employeeId || 'NO-ID'}</td>
                    <td className="px-6 py-4 sticky left-[120px] z-10 bg-white/95 dark:bg-[#080c14]/95 backdrop-blur-md border-r border-slate-200 dark:border-slate-700">
